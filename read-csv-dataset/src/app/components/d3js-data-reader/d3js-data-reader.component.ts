...
export class D3jsDataReaderComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sharedService.getDataWithD3().subscribe(
      (res: any) => {
        var tdValues = d3
          .select(res)
          .selectAll('tr')
        ...
        tdValues.forEach((element: any) => {
          if (d3.select(element).selectAll('td').nodes()[1] != undefined) {
            let obj: player = {
              id: id,
              name: (<HTMLElement>d3.select(element).selectAll('td').nodes()[1])
                .innerText,
              club: (<HTMLElement>d3.select(element).selectAll('td').nodes()[3])
                .innerText,
              image: (<HTMLElement>(
                d3.select(element).selectAll('td').nodes()[4]
              )).innerText,
              nationality: (<HTMLElement>(
                d3.select(element).selectAll('td').nodes()[2]
              )).innerText,
            };
            id++;
            let it = this.playersPerTeam.find(
              (f: any) => f.libelle == obj.club
            );
            if (it) {
              it.count = it.count + 1;
            } else {
  ...
  filter(txt: any) {
    if (txt.value != '' && txt.value != null && txt.value != undefined)
      this.players = this.o_players.filter(
        (p) =>
